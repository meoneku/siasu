    ...
    <div class="col-lg-12">
        <div class="card card-primary card-outline">
            <div class="card-header">
                <h5 class="card-title m-0">Data Pendaftaran Seminar Skripsi</h5>
            </div>
                    ...
                    <table class="table table-hover responsive">
                        ...
                        <tbody>
                            @foreach ($seminar as $data)
                                <tr>
                                    <td>{{ $seminar->firstItem() + $loop->index }}</td>
                                        ...
                                        <div class="btn-group">
                                            ...
                                            <div class="dropdown-menu" role="menu">
                                                ...
                                                @if (Auth::guard('admin')->user()->role == 'root')
                                                    <div class="dropdown-divider"></div>
                                                    <a class="dropdown-item" href="/webmin/seminar/pembayaran/{{ $data->id }}"><i class="fas fa-file-invoice-dollar"></i> {{ $data->status_pembayaran === 'BLM' ? 'Telah Membayar' : 'Belum Membayar' }}</a>
                                                    <form action="/webmin/seminar/{{ $data->id }}" method="post" class="d-inline">
                                                        @method('delete')
                                                        @csrf
                                                        <input type="hidden" name="redirect_to" value="{!! URL::full() !!}">
                                                        <button class="btn-link button-delete dropdown-item" data-message="Data Pendaftar seminar {{ $data->mahasiswa->nama }}"><i class="fas fa-trash"></i> Hapus</button>
                                                    </form>
                                                @endif
                                            </div>
                                        </div>
