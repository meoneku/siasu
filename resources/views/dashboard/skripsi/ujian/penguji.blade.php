    ...
    <div class="col-lg-12">
        <div class="card card-primary card-outline">
            <div class="card-header">
                <h5 class="card-title m-0">Data Pendaftar Seminar Hasil Skripsi | Dosen Penguji</h5>
            </div>
            <div class="card-body">
                ...
                <div class="d-flex justify-content-end mt-2 mb-2">
                    @if ($seminar->status != 5)
                        <a href="{{ url('webmin/semhas/penguji') . '/' . $seminar->id . '/add' }}" class="btn btn-primary"><i class="fas fa-plus"></i> Input Penguji</a>
                    @endif
                </div>
                <table class="table table-striped">
                    ...
                    @foreach ($seminar->dosen as $dosen)
                        <tr>
                            ...
                            <td>
                                @if ($seminar->status != 5)
                                    <form action="/webmin/semhas/penguji/{{ $seminar->id }}" method="post" class="d-inline">
                                        @method('delete')
                                        @csrf
                ...
                @if ($seminar->status == 5)
                    <div class="form-group row mt-3">
                        <label for="tanggal_seminar" class="col-sm-2 col-form-label">Tanggal Ujian</label>
                        <div class="col-sm-4">
                    ...
                    <div class="d-flex justify-content-end mb-2">
                        <a href="{{ url('webmin/semhas') }}" class="btn btn-warning"><i class="fa fa-arrow-left"></i> Kembali</a>&nbsp;
                        <a href="/webmin/semhas/penugasan/{{ $seminar->id }}" class="btn btn-success" target="_blank"><i class="fa fa-envelope"></i> Cetak Surat Penugasan</a>
                    </div>
                @else
                    <form action="/webmin/semhas/penerbitan/{{ $seminar->id }}" method="post" enctype="multipart/form-data" class="mb-2 mt-3">
                        @method('put')
                        @csrf
